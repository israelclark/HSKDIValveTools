...
namespace HSKDIValveTools
{
    ...
    public class MyCommands
    {
        ...
        static public void NumberValves()
        {
            ...
            PromptEntityOptions peo = new PromptEntityOptions("\nSelect the Controller Block or to Resume Previous Count, Select the Designator to Resume From: ");
            peo.SetRejectMessage("\nObject must be a Block or MLeader.");
                ...
                using (tr)
                {
                    ...
                    if (obj.GetType().ToString() == "Autodesk.AutoCAD.DatabaseServices.BlockReference")
                    {
                        ...
                        if (br != null)
                        {
                            ...
                            else if (blockName.ToUpper().Contains("ICO_V") || blockName.ToUpper().Contains("ICO_EXV"))
                            {
                                foreach (ObjectId atId in br.AttributeCollection)
                                {
                                    obj = tr.GetObject(atId, OpenMode.ForRead);
        ...
        public void SizeValves()
        {
            ...
            try
            {
                ...
                {
                    MessageForAdding = "Select blocks or multileaders: "
                };
                            ...
                            foreach (ObjectId atid in mbtr)
                            {
                                ...
                                if (atid.ObjectClass.IsDerivedFrom(RXClass.GetClass(typeof(AttributeDefinition))))
                                {
                                    ...
                                    if (attdef.Tag.ToUpper() == "FLOW") // check your existing tag
                                    {
                                        ...
                                        {
                                            new HSKDICommon.ListPair(attRef.Tag, attRef.TextString)
                                        };
                                        int flow = 0;
                                        if (myAtts.Count > 0 && attRef.Tag.ToUpper() == "FLOW")
                                        {
                                            ...
                                            CalculateValveSize(targetAttrib, flow);     
                                        }
        ...
        private static void CalculateValveSize(HSKDICommon.ListPair targetAttrib, double flow)
        {
            ...
            else targetAttrib.textString = "3\'\'";
        }
